...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.purchaseOrdenItem.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.purchase-orden-items.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('purchase_order') ? 'is-invalid' : '' }}" name="purchase_order_id" id="purchase_order_id">
                    @foreach($purchase_orders as $id => $purchase_order)
                        <option value="{{ $id }}" {{ old('purchase_order_id') == $id ? 'selected' : '' }}>{{ $purchase_order }}</option>
                    @endforeach
                </select>
